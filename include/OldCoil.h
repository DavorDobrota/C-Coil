#ifndef GENERAL_COIL_PROGRAM_OLDCOIL_H
#define GENERAL_COIL_PROGRAM_OLDCOIL_H
...
public:
    ...
    Type a;		//OldCoil thickness
    Type b;		//OldCoil length
    Type d;		//OldCoil winding wire thickness (approximate)
    ...
    OldCoil (Type _I, Type _R, Type _a, Type _b, Type _d, int _nA, int _nB, int _nFi, int _N, bool _isEM, Type _f, Type _Ro){
    ...
    Type ForceCalcBetweenCoilsAmpZ(Type dist, OldCoil coil2){
    ...
    Type ForceCalcBetweenCoilsGradZ(Type dist, OldCoil coil2){
        return -GzCalc(dist, 0.0) * coil2.mM;
    }
    Type ForceCalcBetweenCoilsMomentZ(Type dist, OldCoil coil2){
        return ((6*Mi * mM * coil2.mM) / pow(dist, 4));
    }

    //Interaction between coils: induction
    Type MutualInductanceCalc(Type dist, OldCoil sec, bool automatic, Type precision){
    ...
    Type MutualInductanceGeneralCalc(OldCoil sec, Type distance, Type offAxis, Type alpha, Type beta, bool automatic, Type precision){
        if (automatic == true){
            //setting precision
    ...
    Type VoltageOnSecondNoLoad (Type dist, OldCoil coil2){
...
#endif //GENERAL_COIL_PROGRAM_OLDCOIL_H
