...
{
    int chunkSize = calculateChunkSize(pointVectors.size());
    ...
    for(size_t i = 0; i < (size_t)std::ceil((double)pointVectors.size() / (double)chunkSize); i++)
    {
        g_threadPool.push(
                calcThread,
                std::ref(*this),
                ...
                i * chunkSize, std::min((i + 1) * chunkSize, pointVectors.size())
        );
    }
